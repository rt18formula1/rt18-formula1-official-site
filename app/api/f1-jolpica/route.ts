...
async function getSessionResults(year: number, round: number, session: string): Promise<any> {
  try {
    ...
    // 利用可能なセッションタイプ
    const availableSessions = ['sprint', 'qualifying', 'race'];
    
    if (!availableSessions.includes(session)) {
      // FPセッションなどはデータが利用できない
      throw new Error(`Session type '${session}' is not available in current API`);
    }
